...
// Statuts considérés comme "signés" (dès qu'un devis est accepté, il compte
// dans le CA, quelle que soit la facturation ultérieure).
const STATUTS_SIGNES = ['accepte', 'acompte_facture', 'facture_solde'];

// === Calculs depuis state.fichesList ===
function computeMonthKpis(d) {
  const fiches = (state.fichesList || []).filter(f => STATUTS_SIGNES.includes(f.statut));
...
// === Rendu principal ===
// Labels affichés dans le widget "Top formules" pour les 5 typeId legacy.
// Utilisé aussi en fallback si une formule libre custom référence un typeId
// mais n'a pas d'entrée dans state.bibFormules.
const TYPE_LABELS = {
  'privat-full':      { emoji: '🎭', label: 'Privatisation show + repas' },
  'privat-salle':     { emoji: '🏢', label: 'Privatisation sans show' },
  'atelier-cocktail': { emoji: '🍸', label: 'Atelier cocktail' },
  'formation-impro':  { emoji: '🎬', label: 'Formation impro' },
  'groupe-classique': { emoji: '🎉', label: 'Soirée Palace classique' }
};

// Agrège les formules vendues (fiches signées) sur le mois courant.
// Retourne un top N trié par CA décroissant, avec CA, nb ventes, part.
function computeTopFormulesMonth(d, topN = 5) {
  const fiches = (state.fichesList || []).filter(f =>
    STATUTS_SIGNES.includes(f.statut) && inMonth(f.dateEvent, d)
  );
  const acc = new Map();   // key = libId || typeId → { key, label, emoji, ca, count, isCustom }
  fiches.forEach(f => {
    const libIds = Array.isArray(f.formulesLibIds) ? f.formulesLibIds : [];
    const types = Array.isArray(f.formulesTypes) ? f.formulesTypes : [];
    const nbBlocs = Math.max(1, types.length || libIds.length || 1);
    // Répartir le CA équitablement entre blocs de la fiche
    const caParBloc = (f.totalHT || 0) / nbBlocs;
    // Combine libIds prioritaire, sinon typeIds
    const keys = [];
    for (let i = 0; i < nbBlocs; i++) {
      keys.push(libIds[i] || types[i] || 'autre');
    }
    keys.forEach(k => {
      const cur = acc.get(k);
      if (cur) {
        ...
        // Résolution du label
        let label, emoji, isCustom;
        const customFormule = k && k.startsWith('fl_') ? (state.bibFormules || []).find(x => x.id === k) : null;
        if (customFormule) {
          label = customFormule.nom;
          emoji = customFormule.tag || '🧩';
          isCustom = true;
        } else if (TYPE_LABELS[k]) {
          label = TYPE_LABELS[k].label;
          emoji = TYPE_LABELS[k].emoji;
          isCustom = false;
        } else {
          label = k || 'Autre';
          emoji = '📄';
        ...
        acc.set(k, { key: k, label, emoji, ca: caParBloc, count: 1, isCustom });
      }
    });
  });
  const arr = [...acc.values()].sort((a, b) => b.ca - a.ca);
  const totalCa = arr.reduce((s, x) => s + x.ca, 0);
  return {
    items: arr.slice(0, topN).map(x => ({ ...x, part: totalCa > 0 ? x.ca / totalCa * 100 : 0 })),
    totalCa,
...
export async function renderAccueil() {
  ...
  root.innerHTML = `
    ...
    <section class="accueilBlock">
      ...
      ${renderTopFormulesWidget(currentMonth)}
    </section>
...
function renderTopFormulesWidget(d) {
  const top = computeTopFormulesMonth(d, 5);
  if (top.items.length === 0) {
    return `
      <div class="topFormulesWidget empty">
        <h3>🏆 Top formules du mois</h3>
        <p class="legend" style="margin:6px 0 0 0">Aucune vente signée ce mois-ci — les formules seront rankées dès la première signature.</p>
      </div>
    `;
  }
  const maxCa = top.items[0].ca;
  return `
    <div class="topFormulesWidget">
      <div class="topFormulesHeader">
        <h3>🏆 Top formules du mois</h3>
        <span class="topFormulesTotal">${top.totalFiches} fiche${top.totalFiches > 1 ? 's' : ''} · ${fmt(top.totalCa)} de CA</span>
      </div>
      <ol class="topFormulesList">
        ${top.items.map((x, i) => `
          <li class="topFormulesItem${i === 0 ? ' first' : ''}">
            <span class="topFormulesRank">${i + 1}</span>
            <span class="topFormulesEmoji">${x.emoji}</span>
            <div class="topFormulesLabel">
              <div class="topFormulesName">${escapeHtml(x.label)}${x.isCustom ? '<span class="topFormulesCustomBadge">composée</span>' : ''}</div>
              <div class="topFormulesBar">
                <div class="topFormulesBarFill" style="width:${maxCa > 0 ? (x.ca / maxCa * 100) : 0}%"></div>
              </div>
            </div>
            <div class="topFormulesStats">
              <div class="topFormulesCa">${fmt(x.ca)}</div>
              <div class="topFormulesSub">${x.count} vente${x.count > 1 ? 's' : ''} · ${x.part.toFixed(0)}% du CA</div>
            </div>
          </li>
        `).join('')}
      </ol>
