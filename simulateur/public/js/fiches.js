...
function buildIndexEntry(fiche) {
  ...
  if (Array.isArray(blocs) && blocs.length > 0) {
    ...
    formulesLibIds = blocs.map(b => b.formuleLibId || null).filter(Boolean);
...
export function readCurrentForm() {
  ...
  return {
    ...
    linkedProspectId: window._currentLinkedProspectId || null,
    config: {
...
export function writeFormFromFiche(f) {
  ...
  window._currentLinkedProspectId = f.linkedProspectId || null;
  if (f.config) {
...
export function refreshStatutBadge() {
  ...
  const labels = {
    ...
    accepte: 'accepté', acompte_facture: 'acompte facturé',
    facture_solde: 'facture du solde', refuse: 'refusé'
  };
  ...
  sel.classList.remove('statut-brouillon', 'statut-envoye', 'statut-accepte', 'statut-acompte_facture', 'statut-facture_solde', 'statut-refuse');
...
export function refreshFichesSelect() {
  ...
  sorted.forEach(f => {
    ...
    const statut = (f.statut === 'accepte' || f.statut === 'acompte_facture' || f.statut === 'facture_solde') ? '✓'
      : f.statut === 'refuse' ? '✗'
      : f.statut === 'envoye' ? '→'
      : '·';
...
export function newFiche(askConfirm = true) {
  ...
  writeFormFromFiche({
    ...
    config: {
      // Nouvelle fiche = bloc vierge à 0 € (aucune formule, aucun item).
      // L'utilisateur choisit ensuite sa formule dans la combobox.
      day: 'vendredi', periodeOverride: 'auto',
      formules: [{
        blocId: newBlocId(),
        formuleId: null,
        formuleLibId: null,
...
export async function saveFiche() {
  ...
  // Capture l'état AVANT save pour détecter le changement de statut (sync CRM)
  const oldFiche = state.fichesList.find(f => f.id === id) || null;
  try {
    ...
    if (window._pendingProspectLink) {
      ...
      // Sync auto Simulateur → CRM (création prospect si absent, MAJ statut sinon).
      // Fire-and-forget : ne bloque pas le flow save. Après création/liaison, on
      // persiste linkedProspectId sur la fiche via un 2e putFiche silencieux.
      syncFicheToProspect(saved, oldFiche).then(async res => {
        if (res.prospectId && !saved.linkedProspectId) {
          try {
            const patched = { ...saved, linkedProspectId: res.prospectId };
            await putFiche(saved.id, patched);
          } catch (e) { console.warn('persist linkedProspectId échoué', e); }
        }
      }).catch(e => console.warn('syncFicheToProspect', e));
    }
...
export function refreshDashboard() {
  ...
  const counts = { brouillon: 0, envoye: 0, accepte: 0, acompte_facture: 0, facture_solde: 0, refuse: 0 };
  let caAccepte = 0;
  const statutsSignes = ['accepte', 'acompte_facture', 'facture_solde'];
  state.fichesList.forEach(f => {
    counts[f.statut || 'brouillon'] = (counts[f.statut || 'brouillon'] || 0) + 1;
    if (statutsSignes.includes(f.statut) && typeof f.totalHT === 'number') {
      caAccepte += f.totalHT;
    }
  });
  const nbSignes = counts.accepte + counts.acompte_facture + counts.facture_solde;
  $('dashTotal').textContent = total;
  $('dashBrouillon').textContent = counts.brouillon;
  $('dashEnvoye').textContent = counts.envoye;
  $('dashAccepte').textContent = nbSignes;
  const enJeu = counts.envoye + nbSignes + counts.refuse;
  $('dashTaux').textContent = enJeu > 0 ? Math.round(nbSignes / enJeu * 100) + '%' : '—';
